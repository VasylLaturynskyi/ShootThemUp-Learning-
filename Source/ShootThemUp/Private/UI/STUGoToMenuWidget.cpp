...
void USTUGoToMenuWidget::NativeOnInitialized()
{
	...
	if (GoToMenuButton)
	{
		GoToMenuButton->OnClicked.AddDynamic(this, &USTUGoToMenuWidget::OnGoToMenu); // Підписуємося на подію натискання кнопки
	}
...
void USTUGoToMenuWidget::OnGoToMenu()
{
		...
		{
			UE_LOG(LogUSTUGoToMenuWidget, Error, TEXT("Menu level name is NONE"));
			return;
		}
