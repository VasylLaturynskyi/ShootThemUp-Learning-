...
void USTUGameOverWidget::NativeOnInitialized()
{
    ...
    {
        ResetLevelsButton->OnClicked.AddDynamic(this, &USTUGameOverWidget::OnResetLevel);
    }
...
{
    if (!GetWorld() || !PlayersStatBox) return;
...
{
	//const FName CurrentLevelName = "Test Level";
    
    // Отримує ім'я поточного рівня, на якому знаходиться гравець.
    const FString CurrentLevelName = UGameplayStatics::GetCurrentLevelName(this);
    // Відкриває рівень з ім'ям CurrentLevelName, тобто перезапускає поточний рівень гри
