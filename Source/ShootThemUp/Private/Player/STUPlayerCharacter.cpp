...
{
	// Перевіряємо, чи сфера колізії камери перекривається з капсулою персонажа (тобто камера "всередині" персонажа)
	// Якщо так — приховуємо меш персонажа для власника (гравця), щоб уникнути візуальних артефактів (наприклад, коли камера заходить у голову персонажа)
